  ...
  <section v-if="state.isShow">
    <el-drawer v-model="state.isShow" direction="rtl" :size="state.drawerSize">
      <template #header="{ close, titleId, titleClass }">
        <h4 :id="titleId" :class="titleClass">{{ state.conf?.formConf?.title }}</h4>
        <el-button-group>
          <el-button @click="state.drawerSize = '390px'" icon="Cellphone" title="手机"> </el-button>
      
          <el-button @click="state.drawerSize = '100%'" icon="Monitor" title="电脑"></el-button>
        </el-button-group>  
...
const FormParser = defineAsyncComponent(() => import('../AiFormParser/index.vue'))
