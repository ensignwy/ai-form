<template>
  <el-form-item v-if="props.data.expression !== undefined" label="计算公式">
    <div
      @click="state.expDialogVisible = true"
  ...
  >
      ...
      <div class="calc-preview" :class="{ error: !state.expValid }">
        ...
        <div class="preview-actions">
          <el-icon
            ...
            @click="state.expressionTemp.pop()"
          >
            <PriceTag
          /></el-icon>
          <el-icon @click="state.expressionTemp = []"> <Delete /></el-icon>
        </div>
      </div>
      <div class="calc-tip">
    ...
    <template #footer>
      <span class="dialog-footer">
        ...
        <el-button type="primary" @click="checkExpression" size="small"
          >确 定</el-button
        >
      </span>
    </template>
...
const calculateCmps = computed(() => {
  ...
  const loop = (data, parent) => {
    ...
    if (["el-input-number", "ai-amount"].includes(data.tag)) {
...
const reloadExpressionTemp = () => {
  const isValid = (d) => {
    const target = calculateCmps.value.find(
      (cmp) => cmp.vModel === d.vModel && cmp.label === d.label
    );
    return target ? true : false;
  };
  state.expressionTemp = props.data.expression.map((t) => {
    return typeof t === "string" || isValid(t)
      ? t
      : { vModel: t.vModel, label: "无效的值" };
  });
