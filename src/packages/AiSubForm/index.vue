  ...
  <div class="ai-table-box-parent">
    <div class="ai-table-box" :class="[props.config.type]">
      ...
      >
        ...
        <!-- 组件列 -->
        <el-table-column
          v-for="(head, cindex) in state.tableData"
          :key="head.vModel"
        ...
        >
          ...
          <template #default="{ $index, row }">
            ...
            <template
              v-if="
                ['el-select', 'el-checkbox-group', 'el-radio-group'].includes(
                  head.tag
                )
              "
            >
              <el-select
                ...
                placeholder="请选择"
                :multiple="
                  head.tag === 'el-checkbox-group' ||
                  getConfById(head.vModel).multiple
                "
            ...
            <template v-else-if="head.tag === 'el-upload'">
              <el-upload
                ...
                :on-success="
                  (res) =>
                    onUploadSuccess(res, state.tableFormData[$index][cindex])
                "
            ...
            <div
              class="error-tip"
              v-show="!state.tableFormData[$index][cindex].valid"
            >
              不能为空
            </div>
          </template>
      ...
      <template v-if="props.config.type === 'list'"
        >{{ modelValue }}
        <div
          v-for="(row, rindex) in state.tableFormData"
          :key="rindex"
          class="list-row"
        >
          ...
          <div
            ...
            :key="cindex"
            class="row-item"
            :class="{ error: !state.tableFormData[rindex][cindex].valid }"
          >
      ...
      <div
        class="list-summary"
        v-if="props.config.type === 'list' && props.config['show-summary']"
      >
        <div style="padding: 6px 12px; float: left">合计</div>
        <div style="overflow: hidden; padding-top: 6px">
          <div v-for="(val, name) in state.listSummation" :key="name">
            {{ val.label }}：{{ val.sum }}
          </div>
        </div>
      </div>
      <div class="actions">
        <el-button @click="addRow" type="text">
          <el-icon @click="removeRow(rindex)"><Plus /></el-icon>
...
onMounted(() => {
  state.tableData =
    props.config.type === "table" ? filterProps() : props.config.children;
...
const onUpdate = () => {
  const newVal = state.tableFormData.map((row) =>
...
const onFormDataChange = (rowIndex, colIndex, tag) => {
  if (state.isAddRow || tag === "ai-calculate") return;
...
const getCmpValOfRow = (row, key) => {
  // 获取数字相关组件的输入值
  const isNumCmp = (tag) =>
    ["ai-amount", "ai-calculate", "el-input-number", "el-slider"].includes(tag);
...
const getListSummaries = () => {
  state.tableData.forEach((row) => {
    const isNumCmp = (tag) =>
      ["ai-amount", "el-input-number", "el-slider"].includes(tag);
    if (!isNumCmp(row.tag)) return;
    const sum = state.tableFormData.reduce(
      (sum, d) => sum + getCmpValOfRow(d, row.vModel),
      0
    );
...
const getTableSummaries = (param) => {
  ...
  columns.forEach((column, index) => {
    ...
    const sumVal = data.reduce(
      (sum, d) => sum + getCmpValOfRow(d, column.property),
      0
    );
...
.ai-table-box-parent {
  ...
  :deep(.ai-table-box.table) {
    ...
    .el-table
      .el-table__body
      td.el-table__cell:not(:first-child):hover::before {
